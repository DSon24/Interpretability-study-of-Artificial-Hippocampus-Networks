...
## Progress update — 16 Sep 2026: Table 8 stale artifact regenerated

The committed `results/run_3b_gdn/05_table8_rq3.json` was stale: it still used the old `@8K` field labels and therefore reported `n=0` / NaN for the RQ3 correlations. The current `04b_joined_retention_task.json` already contains the corrected prompt-end fields, so Table 8 was regenerated without a new model run.

Current GDN Table 8 results (n=60):
- target rank @ prompt end: rho = -0.0997, p = 0.4484
- target mass @ prompt end: rho = 0.1867, p = 0.1532
- readout entropy @ prompt end: rho = 0.1942, p = 0.1370

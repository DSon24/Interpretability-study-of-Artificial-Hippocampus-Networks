...
| Per-layer recomputation of the control battery | **done, no GPU** — the pooled battery averaged a degenerate layer-9 readout with two working layers. Layer 27 is above chance and 5.03× on C2; layer 9 fails C4 | `results/run_3b_gdn/04c_per_layer_controls.json`, Hannah, 2 Sep |
...
## Findings from the 2 Sep per-layer re-analysis

No new GPU time. `per_layer_controls.py` recomputes the C1–C4 battery from the 624 rows
already in `04_retention_rows.json`, within layer instead of pooled across layers, with
bootstrap CIs. Output: `results/run_3b_gdn/04c_per_layer_controls.json`.

**1. The pooled battery averaged a degenerate instrument together with two working ones.**

| layer | readout entropy | C1 evicted rank (95% CI) | C2 ratio | C2 win rate | C4 | rows < chance |
|---:|---:|---|---:|---:|---|---:|
| 9 | **0.088 nats** | 84,406 [78,897 – 100,206] | 0.71× | 46% | **FAIL** | 37.5% |
| 18 | 5.03 | 114,104 [98,440 – 120,976] | 0.56× | 38% | pass | 36.1% |
| 27 | 5.76 | **68,132 [61,897 – 74,566]** | **5.03×** | **75%** | pass | **61.1%** |

Chance rank is 75,968; uniform entropy over the 151,936-token vocabulary is 11.93 nats.

**2. Layer 9's readout has collapsed.** A median entropy of 0.088 nats means the readout
is putting essentially all its mass on a single token — it is not noisy, it is
*confidently wrong*, which is a different failure mode from "at chance" and is consistent
with the structural-continuation problem diagnosed in the 19–20 Aug run (Finding 4). It
also **fails C4**: the in-window ceiling (118,723) is worse than the evicted floor
(84,406), which is exactly the diagnostic the 18 Aug pilot flagged as proof that a
measurement is not measuring retention. Layer 9 should not be pooled with the others, and
arguably should not be in the layer set at all.

**3. Layer 27 is a working instrument, but a weak one.** C1's bootstrap CI
[61,897 – 74,566] sits entirely below chance, so the needle *is* found above chance.
C2 is 5.03× with the needle beating its distractor in 63 of 84 evicted rows. Both are
real and both are directionally right. **Do not oversell this**: a median rank of 68,132
out of 151,936 is the 45th percentile of the vocabulary — detectable, not usable — and
5.03× is still below C2's 10× bar. What layer 27 establishes is that the readout is not
uniformly broken, not that RQ2 is answerable as it stands.

**4. In-window needles read well; evicted ones do not.** The best rows in the entire sweep
are in-window: rank 776–1,812 out of 151,936, i.e. the top ~1%. At layer 18 the in-window
median is 40,644 against an evicted median of 114,104. This also **retires the caveat on
the 21 Aug diagnosis**, which measured in-window rank at 95,949 using the plain logit lens
because no lens argument was available on that box; through the fitted J-Lens the same
condition is far stronger.

**5. What this does to the C1 diagnosis — the candidate list has changed again.**

- **(a) undersampling** — still ruled out (map stability).
- **(b) prompt construction** — **weakened.** In-window needles read at rank ~800, so the
  NIAH prompt is not fatally malformed. The missing answer prefix (§Next steps 5) may still
  cost signal, but it cannot be the whole story.
- **(c) content does not survive compression** — **strengthened.** Strong in-window signal
  and weak evicted signal at layers 18 and 27 is the signature you would expect if
  compression destroys linearly-decodable content. Note this is *not* the same claim as
  "AHN is a recency mechanism," which the corrected C3 does not support.
- **(d) layer selection — new.** Layer 9 sits below the depth at which this lens carries
  content. Fixing the layer set in advance was the right call methodologically (it avoided
  selection-on-test), but one of the three chosen layers is a broken instrument, and that
  is now a measured fact rather than a judgement.

**6. Caveats on this re-analysis, stated plainly.** It reuses rows collected under the
pre-31-Aug `needle_pos` off-by-four, so eviction distances are ~4 tokens optimistic — the
same shift that moved the pooled battery by under 0.2%. The per-layer C2 counts (84 rows
per layer) differ slightly from Sơn's C2 investigation (92 per layer) because this filters
to unshuffled evicted rows only. Nothing here has been through the pair-baseline correction
that Sơn's work showed C2 needs, so the 5.03× at layer 27 is a *raw* ratio and is subject
to exactly the pair-identity confound documented above. Treat item 3 as "worth re-running
...
per_layer_controls.py         CPU-only: recompute Table 4 within layer, not pooled
notebooks/
...
results/
  run_3b_gdn/                        the run of record — GDN 3B, window 8064, sinks 128
    ...
    04c_per_layer_controls.json        the same battery within layer — see Findings, 2 Sep

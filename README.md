...
| Reproduce published NOWRITE result (38–42% changed) | **done on the Qwen chat template (rerun 21 Sep, all three cells)** — change rate 30.0% GDN / 31.7% DeltaNet / 30.0% Mamba2 (first-line, normalised EM), point estimates below the published band but 95% CIs (e.g. GDN [18.3, 41.7]) include it; ΔF1 −4.10 / −3.96 / −4.63 pts. The earlier 33.3% / +3.34 (GDN) and 41.7% (DN, M2) were raw-prompt figures — [21 Sep rerun](docs/FINDINGS.md#rq1rq3-rerun-with-the-qwen-chat-template-on-all-three-cells--2026-09-21--withdraws-the-cross-cell-conclusion-above) |
...
| RQ1 on LongBench-E HotpotQA (Table 5) | **done, rerun 21 Sep on the Qwen chat template for all three cells** — first-line ΔF1 vs NOWRITE: GDN −4.10, DeltaNet −3.96, Mamba2 −4.63 pts (n=60); only GDN's 95% CI excludes zero. No pairwise contrast survives Holm (DeltaNet vs GDN +0.14, p = 1.0), so the 17–18 Sep "cells differ" claim is **withdrawn** — it mixed prompt formats — [FINDINGS](docs/FINDINGS.md#rq1rq3-rerun-with-the-qwen-chat-template-on-all-three-cells--2026-09-21--withdraws-the-cross-cell-conclusion-above) |
...
| [DeltaNet RQ1 rerun](docs/FINDINGS.md#findings-from-the-deltanet-rq1-rerun-nb03-9-sep) · [Mamba2 RQ1 run](docs/FINDINGS.md#findings-from-the-mamba2-rq1-run-nb03-9-sep) | **Superseded 21 Sep.** These 9 Sep runs used the raw prompt (change rate 41.7% for both, inside the 38–42% band). On the chat template the change rate is 31.7% (DN) and 30.0% (M2), no significant ΔF1 at n=60; the notebook's reproduction check still passes (30–50% tolerance). See the [21 Sep rerun](docs/FINDINGS.md#rq1rq3-rerun-with-the-qwen-chat-template-on-all-three-cells--2026-09-21--withdraws-the-cross-cell-conclusion-above) |
